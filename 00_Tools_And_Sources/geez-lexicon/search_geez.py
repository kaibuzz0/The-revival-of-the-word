...
def load_lexicon():
    """Load the Ge'ez lexicon"""
    lexicon_file = Path(__file__).parent / "geez_lexicon.json"
    
    if not lexicon_file.exists():
        print(f"[!] Lexicon not found: {lexicon_file}")
        print("Run geez_lexicon_builder.py first.")
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        print("Ge'ez Lexicon Search")
        print("Usage: python search_geez.py <search_term>")
        print()
        print("Examples:")
        print("  python search_geez.py God")
        print("  python search_geez.py love")
        print("  python search_geez.py q-d-s")
        print("  python search_geez.py አምላክ")
        sys.exit(0)
    ...
    if not results:
        ...
        print("\nTips:")
        print("  - Try Ge'ez characters (አ, ከ, መ, etc.)")
        print("  - Try transliteration (amlak, Iyesus, Qedus)")
        print("  - Search by root (q-d-s, ʾ-l-k, etc.)")
        print("  - Search by English meaning (God, love, holy)")
        sys.exit(0)
