...
def load_lexicon():
    """Load the Syriac lexicon"""
    lexicon_file = Path(__file__).parent / "syriac_lexicon.json"
    
    if not lexicon_file.exists():
        print(f"[!] Lexicon not found: {lexicon_file}")
        print("Run syriac_lexicon_builder.py first.")
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        print("Syriac Lexicon Search")
        print("Usage: python search_syriac.py <search_term>")
        print()
        print("Examples:")
        print("  python search_syriac.py God")
        print("  python search_syriac.py Jesus")
        print("  python search_syriac.py ܐܠܗܐ")
        print("  python search_syriac.py mary")
        sys.exit(0)
    ...
    if not results:
        ...
        print("\nTips:")
        print("  - Try Syriac characters (ܐ, ܒ, ܓ, etc.)")
        print("  - Try transliteration (Alaha, marya, Yeshu)")
        print("  - Search by root (ʾ-l-h, m-r-ʾ, etc.)")
